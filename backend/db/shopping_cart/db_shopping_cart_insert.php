...
if ($available_stock <= 0) {
    echo json_encode(['success'=>false,'message'=>'Selected size is out of stock']);
    exit;
}
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    if (mysqli_query($conn, $update_sql)) {
        echo json_encode(['success' => true, 'message' => 'Cantidad del producto '.$product_id.' actualizada a '.$new_quantity, 'product_id' => $product_id, 'quantity' => $new_quantity]);
    } else {
    ...
    if (!$query) {
        echo json_encode(['success' => false, 'message' => 'Error al agregar al carrito: ' . mysqli_error($conn)]);
    } else {
        echo json_encode(['success' => true, 'message' => 'Producto '.$product_id.' añadido al carrito', 'product_id' => $product_id]);
    }
}
?>
